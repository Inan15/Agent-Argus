...
The complete-the-declared-set matrix (AI-E5-1 / AI-E6-1 / AR10), each covered below:
  ...
  (8) Story 8.5 / DR-10 — the plan artifacts name their real subject → TC-...-41
...
# ─────────────────────────────────────────────────────────────────────────────
# Member (8) / Story 8.5 (DR-10) — the plan artifacts name the tree they PLANNED
# ─────────────────────────────────────────────────────────────────────────────


def test_plan_artifacts_name_the_tree_they_actually_planned() -> None:
    """TC-ArgusAgent-DOGFOOD-001-41 — Story 8.5/AC2/AC6: honest subject on BOTH plan artifacts.

    ``build_full_repo_plan`` defaults to ``scope_prefix="argus/"``, so the plan is derived
    over THIS repository's own package. Both committed plan artifacts previously carried
    the identical falsehood the proof artifact did — a ``Minions Dogfood`` title, a
    ``tracked minions_core/, excluding minions_core/apaa/`` source line and an
    ``AUTO-GENERATED by minions_core/apaa/dogfood/partition_plan.py`` banner naming a
    module that does not exist. The rendered subject must be derived from the scope the
    plan actually recorded, so the artifact can never name a tree the derivation did not
    read. RED-first is the pre-8.5 committed artifact itself.
    """
    result = build_full_repo_plan(str(_REPO_ROOT))
    # The plan RECORDS its own subject (defaulted, so no existing call site breaks).
    assert result.scope_prefix == "argus/"
    assert result.exclude_prefixes == ("argus/tests/",)
    renders = {
        "partition": render_partition_plan_markdown(result),
        "budget": render_budget_plan_markdown(result),
    }
    committed = {
        "partition": _PARTITION_PLAN.read_text(encoding="utf-8"),
        "budget": _BUDGET_PLAN.read_text(encoding="utf-8"),
    }
    for name in renders:
        for surface, text in ((f"live {name} render", renders[name]), (f"committed {name}", committed[name])):
            lowered = text.lower()
            for claim in ("minions_core/", "minions dogfood —", "tracked `minions_core/`"):
                assert claim not in lowered, (
                    f"FALSE SUBJECT in {surface}: {claim!r} — the plan enumerates "
                    "scope_prefix='argus/' (this repository's own package)"
                )
            # The real generator module is named, and it exists on disk.
            assert "`argus/dogfood/partition_plan.py`" in text, (
                f"{surface} must name the REAL generator module in its provenance banner"
            )
            assert (_REPO_ROOT / "argus" / "dogfood" / "partition_plan.py").exists()
            # The self-scoped weakness is STATED, not left to be inferred.
            assert "self-scoped plan" in lowered, (
                f"{surface} must state that the planned tree is its own package"
            )
            assert "never as independent corroboration" in lowered
            # The subject line is rendered from the RECORDED scope, not hardcoded, and it
            # names ONLY the exclusions the enumerator MEASURABLY applied (8.5 review
            # it.1): a configured prefix that held no tracked file out must never be
            # published as a held-out sub-tree. `argus/tests/` was exactly that — the
            # tests are flat under `tests/`, so the clause asserted a directory that does
            # not exist. Rendering the CONFIGURED set is the defect; this asserts the
            # MEASURED one, which is strictly the stronger claim.
            excluded = ", ".join(f"`{p}`" for p in result.effective_exclude_prefixes)
            tail = f", excluding {excluded}" if excluded else ""
            assert (
                f"- Source files (tracked `{result.scope_prefix}`{tail}): "
                f"**{result.source_file_count}**"
            ) in text
            for prefix in result.exclude_prefixes:
                if prefix not in result.effective_exclude_prefixes:
                    assert f"excluding `{prefix}`" not in text, (
                        f"{surface} renders {prefix!r} as an exclusion, but it held NO "
                        "tracked file out — an exclusion that excluded nothing"
                    )
    # The MEASURED exclusions are the derivation's own measurement, not a re-derivation.
    unfiltered = enumerate_minions_source_files(
        str(_REPO_ROOT), scope_prefix=result.scope_prefix, exclude_prefixes=()
    )
    assert result.effective_exclude_prefixes == effective_exclusions(
        unfiltered, result.exclude_prefixes
    )
    for prefix in result.effective_exclude_prefixes:
        assert any(f.startswith(prefix) for f in unfiltered)
