...
Every case in THIS module constructs an ``AstIndexEntry`` **by hand**, so none of them
needs tree-sitter and the pure heuristic logic over given counts stays UNCONDITIONALLY
tested. The cases that stand up the **real** Story-1.4 substrate via ``build_ast_index``
— together with ``_grammars_or_unevaluable()``, the ``UNEVALUABLE`` guard that makes a
missing grammar a NAMED failure rather than a skip (``DF-14-2-A``) — moved to
``tests/test_vacuous_detector_index.py`` when Story 15.2 split this module at
NFR-M1's ceiling (``DN-15-2-1``). That module's docstring records the boundary and the
