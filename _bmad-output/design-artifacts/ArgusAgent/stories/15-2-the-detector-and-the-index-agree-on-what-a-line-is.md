...
- **CI matrix result: NOT ESTABLISHED at the time of writing.** The work is committed and pushed;
  the `ubuntu-latest` × Python 3.10/3.11/3.12 run had not reported when this record was written,
  so no run id or per-version conclusion is recorded here. Everything above is a **LOCAL**
  (Windows, CPython 3.11.15) measurement. This matters more than usual for this story: the
  non-ASCII separators `\x85`, `\u2028`, `\u2029` are exactly the class that behaves differently
  under a non-UTF-8 locale and a case-sensitive filesystem. The three §0.18 hazards were each
  discharged by a named mechanism, but **discharge by mechanism is not discharge by observation.**
