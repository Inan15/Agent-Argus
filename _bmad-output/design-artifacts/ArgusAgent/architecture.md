...
**Gate-decision enforcement** *(added 2026-08-17 by Story 13.3 / AC1, extending Adjudication-record enforcement above)*. **The rule: the outcome of the >=80%-precision externalization gate is recorded in a CLOSED THREE-MEMBER vocabulary — `CLEARED` / `NOT_CLEARED` / `BLOCKED` — that RAISES on an unregistered member; `NOT_CLEARED` may be recorded ONLY when the measurement RAN (the record is byte-reproducible AND exhaustively adjudicated AND the precision denominator is non-empty), and `BLOCKED` — which is NOT a §5 outcome — may never be rendered, serialized, summarised or committed as *"the gate did not clear"*, in any artifact, in any wording.** *A gate that did not clear because findings were judged and enough of them were false is a **measurement**; a gate whose adjudication has not terminated is an **absence**.* A two-member vocabulary cannot tell a reader which happened, and every downstream surface inherits the ambiguity — permanently, because the artifact outlives the story. **Two further clauses ride with it:** (a) protocol §5's four conditions are reported **individually**, each with its own measured value, its own verdict from a closed verdict vocabulary and its own countable *"what would close it"* — never as one boolean, and `NOT_APPLICABLE` is not a synonym for `MET` (a `CLEARED` decision carrying a non-`MET` condition RAISES at construction, which is §5's *"it may not count it as met by default"* made unexpressible rather than written down); (b) **the result DISCLOSES THE CONCENTRATION OF ITS OWN DENOMINATOR** — contributing members vs. ratified members, per-member finding counts and the distinct rule-class count — **derived, never typed** (`DF-8-5-C` / `AI-E9-7`), in **both** branches, because §5's `N >= 5` is satisfied by member COUNT while the ratio is computed over whichever members contributed, and *the N that gates and the N that contributes are different numbers*. The disclosure is **not** a distribution requirement: the concentration is disclosed, never corrected by narrowing the corpus, which would be a threshold change wearing a hat. It is enforced by **`argus/precision/gate_decision.py`** and **`argus/precision/gate_disclosure.py`** (structural: `GateDecision.__post_init__` raises on an unregistered outcome, on a condition set that is not exactly ~~§5's four in §5's order~~ **AMENDED 2026-08-20 by Story 16.1 / AC1 — STRUCK, never erased (§3.4): §5's conditions in §5's order, a set that is now FIVE and that §5 amends by dated ADDITION — the count is derived from `SECTION_5_CONDITIONS` in the refusal's own message so a further amendment does not require editing a shipped gate's error text**, on `CLEARED` with a non-`MET` condition, on a `BLOCKED` decision carrying no closure path, and on any §5 outcome recorded over a fold that is not evaluable; ~~`decide_gate` raises `VacuousDecisionError` on an empty record or an empty emitted population BEFORE asserting anything about them~~ **AMENDED 2026-08-18 by Story 13.5 / AC5 — STRUCK, never erased (§3.4): `decide_gate` still raises `VacuousDecisionError` on an empty record unconditionally, and still raises on an empty EMITTED POPULATION that carries no positive corpus-read proof; but an empty emitted population accompanied by a `CorpusReadProof` whose every conjunct holds — members audited at their PINNED shas with each staged file proved against the pin by git's own blob hash, source files scanned, test functions scored, two runs byte-identical — is ADMITTED and returns `BLOCKED` with protocol §5's precision condition `UNEVALUABLE`. **The floor is narrowed, never removed, and both directions are guarded** (`TC-ArgusAgent-PRECISION-001-69`), because the unconditional form conflated *"the corpus could not be read"* with *"the corpus was read and nothing was promoted"*, and Epic 14's corrected detector made the second one real: 1,960 in-scope source files at the pins, 5,129 test functions scored, 4,284 advisory findings emitted, **0** promoted to verdict-eligible. As shipped, that outcome was INEXPRESSIBLE by the instrument built to record it) and asserted by **`tests/test_gate_decision.py`** (`TC-ArgusAgent-PRECISION-001-53`..`-64`). **Measured on the day it landed, which is why it is a rule and not a preference:** the committed record held 26 TP/FP dispositions and **5 `BORDERLINE`** whose §4 ladder had not terminated, so the fold was `Unevaluable` — and folding it anyway would have produced a confident, fully-green *"the gate did not clear"* drawn from an adjudication nobody finished, in wording no downstream reader could distinguish from an honest shortfall. `TC-ArgusAgent-DOCS-001-77` asserts this registration is still present. **⚖️ AMENDED 2026-08-20 by Story 16.1 / AC1 — a FIFTH §5 condition: THE DENOMINATOR MUST BE BROAD ENOUGH TO MEAN SOMETHING.** **The added rule: the precision ratio is EVALUABLE only over a population drawn from at least `(VALIDATION_SET_FLOOR_N + 1) // 2` DISTINCT CONTRIBUTING members of the ratified repository corpus; below that, §5's PRECISION condition is recorded `UNEVALUABLE` with the counts that made it so and the outcome is `BLOCKED` with a countable closure path — while the breadth condition's OWN verdict is `MET` or `FAILED`, because it WAS evaluated over a named population and a measured result is not an unobservable one.** Clause (b) above already required the concentration to be DISCLOSED; **this is the clause that lets the record act on it.** It is enforced by **`argus/precision/gate_breadth.py`** (the floor is a FUNCTION of the one locked floor, never the integer it evaluates to; the predicate is pure; the counts are READ from the very `ConcentrationDisclosure` the decision publishes, never recounted, so the threshold and the disclosure cannot disagree) and asserted by **`tests/test_gate_breadth.py`** (`TC-ArgusAgent-PRECISION-001-82`..`-85`), whose populations are GENERATED one per contributing-member count and which assert **where the verdict flips**, not merely that it has two values. **The positional read is repaired in the same change** — `decide_gate` read its own recorded-cleared verdict as `conditions[3].verdict`, correct for four conditions in §5's order and a latent FALSE GREEN the moment §5 is amended by addition, since an index returns a well-formed verdict belonging to another condition with no shape a reader, a schema or a guard could notice; it is now `section_5_condition(conditions, <id>)`, which RAISES (`TC-ArgusAgent-PRECISION-001-80`/`-81`). **The RULE-CLASS arm was derived and deliberately NOT landed** (operator decision, XAgent007, 2026-08-20): measured by two independent instruments, the maximum achievable distinct verdict-eligible rule-class count is **1**, so a floor of ≥2 would be a shutdown rather than a strengthening and a floor of 1 could not fail — filed as `DF-16-1-A`, with the count still disclosed on every decision. **No change-log version was taken:** the amendment sits under the existing V1.3 because the committed record carries 31 human judgements made under it, and re-stamping them would be the re-interpretation `decide_gate`'s own refusal names. **⚖️ AMENDED 2026-08-20 by Story 16.2 / AC3 — a SIXTH §5 condition: THE EVIDENCE THAT GATES MUST COME FROM A PARTITION THE TOOL WAS NEVER TUNED AGAINST.** Recorded as a further dated addition, STRUCK NEVER ERASED (§3.4): the fifth condition's registration above is byte-unchanged and ~~a set that is now FIVE~~ **is now SIX**, still amended only by dated ADDITION with every historical id in its historical position. **The added rule: every corpus member lies in exactly one of three partitions — `sealed` / `open` / `pre-seal` — assigned by a PURE PER-ROW FUNCTION OF ITS PINNED COMMIT SHA plus a prior-output override (a member over which Argus output already existed when the seal was taken is `pre-seal` unconditionally, whatever its sha says); and the precision ratio is EVALUABLE only over a population drawn from at least `(VALIDATION_SET_FLOOR_N + 1) // 2` DISTINCT CONTRIBUTING members lying in the `sealed` partition — the SAME derived floor the breadth arm uses, RESOLVED through the same function and never forked. Below that, §5's PRECISION condition is `UNEVALUABLE` and the outcome is `BLOCKED` with a countable closure path, while the seal condition's OWN verdict is `MET` or `FAILED`, because the provenance of the evidence WAS established over a named population and `UNEVALUABLE` would claim it was unknown.** This is the repository corpus's equivalent of the cartridge corpus's author-blind holdout, and it had to be an ORDERING rather than a labelling, because a repository has no golden key a labeler could be blinded to: the partition is frozen, in code and in git, in a commit that PRECEDES every commit containing Argus output over any member of it. It is enforced by **`argus/precision/gate_seal.py`** (the rule is ONE pure function — no I/O, no clock, no manifest resolution, asserted STRUCTURALLY by an AST walk of the module's own imports; the vocabulary is CLOSED at three and RAISES on an unregistered member; the counts are READ from the very `ConcentrationDisclosure` the decision publishes and from the partitions the decision already carries, never recounted) and by **`tests/corpus/_manifest.py`** (the partition is a DERIVED `@property` on the row and never a stored field — `MANIFEST_FIELDS` stays CLOSED at 9 and `TC-ArgusAgent-PRECISION-001-22` stays green unedited — so it cannot change without changing the PIN, which changes which bytes are audited and is refused at construction; and the 40-hex pin check is HOISTED to validate EVERY row, closing a measured gap in which a candidate row accepted `commit_sha="NOT-A-SHA"` in silence). It is asserted by **`tests/test_gate_seal.py`** (`TC-ArgusAgent-PRECISION-001-87`..`-94`), whose populations are GENERATED with **breadth pinned TRUE so the seal is the only term that can move the answer**, driven to both verdicts and asserted at the exact count where the verdict flips. **It PARTITIONS; it does not NARROW:** `VALIDATION_SET_FLOOR_N` stays 5, `eligible_member_count()` stays 5, no member is dropped or re-weighted, and every finding from every partition stays recorded and stays disclosed — the seal governs what may GATE, never what is REPORTED. Both narrowing designs were tested by execution and both `raise`, which is the correct refusal: a filter NARROWS, which §5 and Story 13.3 / AC5 forbid, while a CONDITION REQUIRES. **A post-seal detector change must CITE its partition** — any commit touching a declared detector-tuning path that is not an ancestor of the seal commit carries an `Evidence-partition: sealed | open | none` trailer, a disclosure rather than a prohibition, because the value of a holdout is the comparison ACROSS partitions and the comparison needs each change to say which side it learned from. **Opening the sealed partition is a single recorded §6 R2-class operator act and never a side effect of running the harness: no code path in this repository opens it.** **`DF-16-1-B` fired on this change and was discharged FIRST**, in its own commit: `argus/precision/gate_decision.py` stood at 1,197 of NFR-M1's 1,200 lines and was split along its cohesion boundary into `argus/precision/gate_conditions.py` (what a §5 condition IS) and `argus/precision/gate_evidence.py` (what one is MEASURED FROM), 319 lines moved byte-for-byte with every symbol re-exported and no import line anywhere changed. **No change-log version was taken here either:** the same operator decision of 2026-08-20 applies unchanged — the 31 human judgements of 2026-08-17 keep their V1.3 provenance and `adjudication-record.json` was not regenerated. **⚖️ AMENDED 2026-08-20 by Story 16.3 / AC1 — a SEVENTH §5 condition: A DETECTOR THAT FINDS NOTHING HAS NOT PASSED.** Recorded as a further dated addition, STRUCK NEVER ERASED (§3.4): the fifth and sixth conditions' registrations above are byte-unchanged and ~~a set that is now SIX~~ **is now SEVEN**, still amended only by dated ADDITION with every historical id in its historical position. **Measured before it was argued:** driven through the shipped `decide_gate` at `1ecf618`, a population of **exactly three findings — one per sealed member, all adjudicated TP — returned `CLEARED`** at precision `1/1` with all six conditions `MET` and an outcome sentence reading *"Clearing authorises ATTESTED externalization."* So did four. `UNEVALUABLE` closed the **empty**-denominator hole and Story 13.5 made *"the corpus was read and nothing was promoted"* expressible; neither closes the **tiny** one. **The added rule: the precision ratio is EVALUABLE only over a VERDICT-ELIGIBLE population of at least `ceil(q / (q - p))` adjudicated findings for the gate threshold `PRECISION_GATE_THRESHOLD = p/q` — the smallest denominator at which ">=80%" is not silently "100%", which at the locked 4/5 is 5. Below that, §5's PRECISION condition is `UNEVALUABLE` with the counts that made it so and the outcome is `BLOCKED` with a countable closure path, while the yield condition's OWN verdict is `MET` or `FAILED`, because the population WAS counted and `UNEVALUABLE` would claim its size was unknown.** The floor is **DERIVED, never typed**, and stated in the GENERAL form because `ceil(q / (q - p))` equals `q` only when `q - p == 1` — at `5/7` it is 4 and at `7/9` it is 5, verified against brute force — so `threshold.denominator` would be correct by coincidence at exactly the one threshold shipped, which is Story 16.1's *"strict majority"* correction repeated one story later. **`VALIDATION_SET_FLOOR_N` is also 5 and the equality is a disclosed COINCIDENCE, not a derivation**: one counts members that must EXIST, the other is the smallest denominator at which a ratio threshold is the threshold it is written as. It is enforced by **`argus/precision/gate_yield.py`** (the floor is a FUNCTION of the threshold, which arrives as an ARGUMENT; the arithmetic is exact integer ceiling division, never a float at the boundary; the count is READ from the very `ConcentrationDisclosure` the decision publishes and the breadth and seal arms read, never recounted; a threshold admitting no false positive at any denominator RAISES `VacuousYieldFloor`) and asserted by **`tests/test_gate_yield.py`** (`TC-ArgusAgent-PRECISION-001-95`..`-100`), whose populations are GENERATED with **breadth AND the seal pinned TRUE so the population SIZE is the only term that can move the answer** — the third form of the lockstep trap 16.1 and 16.2 each hit — and which assert **where the verdict flips**, at exactly the derived floor. **It is NOT a recall condition and the OI1 row is amended in terms rather than left to be inferred:** `recall = TP / (TP + FN)` needs `FN`, which is unknowable over a repository corpus; this condition's only inputs are the counted population and the threshold, so it is a claim about the RESOLUTION of the measurement taken and never about what was missed. **That boundary is enforced STRUCTURALLY** — `TC-ArgusAgent-PRECISION-001-99` walks `gate_yield.py`'s own AST and fails if it imports a recall symbol or references any `FN` / co-occurrence / bench-content quantity, and the guard is itself driven RED by adversarial variants that add one. A floor derived from bench content would be recall with an estimated denominator, and re-opening OI1 is an **operator act**. **It COMPOSES with the fifth and sixth conditions and replaces neither:** quantity without breadth still fails (40 findings from one sealed member stays `BLOCKED`), and breadth without quantity now fails too. **It governs what may GATE, never what is REPORTED:** every finding from every member stays recorded and stays disclosed, `VALIDATION_SET_FLOOR_N` stays 5, `MANIFEST_FIELDS` stays closed at 9, `GATE_OUTCOMES` at three and `CONDITION_VERDICTS` at four. **⚠️ The PRE-ROUND DISCLOSURE rides on the condition itself** (`YIELD_PROVENANCE_DISCLOSURE`, re-derived from the committed artifacts by `-100`): the corrected detector emitted **4,284 findings across all five ratified members and promoted ZERO**, and the only population that ever exceeded this floor was the 2026-08-16 set of 31 produced under the corroboration rule Epic 14 refuted and adjudicated 0 TP / 26 FP / 5 BORDERLINE — so the committed `MET` must not be read as *"the detector currently yields 31."* **`tests/test_gate_decision.py` was the SPLIT-FIRST trigger this time and was discharged FIRST**, in its own commit: it stood at **1,191 of 1,200** with **no ledger entry at all**, and the six guards over the ARTIFACT the decision publishes (`-59`..`-64`) moved byte-for-byte to **`tests/test_gate_decision_artifact.py`** — the test-side mirror of 16.2's production split. **No change-log version was taken a third time:** the same operator decision of 2026-08-20 applies unchanged — the 31 human judgements keep their V1.3 provenance and `adjudication-record.json` was not regenerated. **⚖️ AMENDED 2026-08-23 by Story 16.5 — WHO JUDGED IS NOW DERIVED AND PUBLISHED, AND IT IS A DISCLOSURE, NOT AN EIGHTH CONDITION.** Recorded as a further dated addition, STRUCK NEVER ERASED (§3.4): every registration above is byte-unchanged, and — unlike the three amendments preceding it — **this one does NOT append a §5 condition. `SECTION_5_CONDITIONS` stays at SEVEN, `argus/precision/gate_conditions.py` is byte-unchanged, `GateDecision.precision_evaluable` keeps exactly FOUR conjuncts and `_precision_condition` gains no branch.** **Measured before it was argued:** `GateDecision` had carried `adjudicators: tuple[str, ...]` since Story 13.3 and published it under `adjudication_record.adjudicators` ever since, and at `52143eb` **ZERO** assertions anywhere in `tests/**` or `scripts/**` closed over it and **no** surface rendered it beside the precision figure — so a reader who wanted to know whether the precision figure was judged by the people who wrote the tool had to open a JSON artifact, find a nested list of `"<who> (<role>)"` strings thirty lines from the number, and know that protocol §2 registers three roles and that one of them *"SHOULD be outside the implementing team"*. **The added rule: the decision DERIVES, from the registered adjudicator ids on the LIVE rows of the committed adjudication record, a member of a CLOSED FOUR-MEMBER vocabulary — `NOT_ESTABLISHED` / `NOT_INDEPENDENT` / `SECOND_REVIEWER_INTERNAL` / `EXTERNAL_ADJUDICATOR_PARTICIPATED`, which RAISES on an unregistered member — and publishes it TWICE: as a structured `independence` block on the payload, and as a clause ON THE `gate_status` SENTENCE ITSELF, immediately after the precision figure, in ALL THREE of that renderer's branches and on EVERY one of the four renderers `precision_gate_status` can return.** The second placement is the rule; the first is a convenience for machines. *Two facts in the same file thirty lines apart are separable by copy-and-paste, and the sentence a reader actually quotes said nothing about who judged.* ⛔ **`NOT_ESTABLISHED` and `NOT_INDEPENDENT` are NOT synonyms** — *nobody judged it* and *the author judged all of it* are different findings, the distinction `BLOCKED` vs `NOT_CLEARED` already makes one level in (`AI-E11-1`). ⛔ **The status is a claim about the ADJUDICATION THAT WAS PERFORMED, never about §2's roster, and the published sentence says so in its own words** — the QA Lead has been FILLED since 2026-08-22 and has authored ZERO dispositions, so a sentence reading *"QA Lead: absent"* would be read as *unfilled* and would be false: the `DF-9-2-B` false-subject class on the surface that publishes the gate. It is enforced by **`argus/precision/gate_independence.py`** (PURE — no I/O, no clock, no module-level repository-only path, because every input arrives as an argument; the three role names are DESTRUCTURED out of `PROTOCOL_ADJUDICATOR_ROLES` rather than re-typed, so a fourth registered role fails at import; ids are parsed by the EXISTING `adjudicator_role`, never a second regex; the derived value is READ off the `adjudicators` tuple `decide_gate` already computes, never a second count over the record) and asserted by **`tests/test_gate_independence.py`** (`TC-ArgusAgent-PRECISION-001-105`..`-112`) and **`tests/test_gate_decision_artifact.py`** (`-113`, which RE-DERIVES the committed artifact's block from the committed adjudication record so the summary cannot drift from the evidence it summarises). **The ONE renderer is not forked** (AR7): `precision_gate_status_for` gains a single optional keyword defaulting so every existing caller renders byte-identical bytes, and the adjudication fold and §5's three arm renderers each FORWARD that one opaque keyword and nothing else — none of them derives, parses or words the note, and none imports this module. **Byte-stability was PROVEN BY RENDERING, not by reading the diff:** 26 surfaces rendered against the pre-story tree and against this one with the keyword omitted, byte-identical. **It GATES NOTHING, and that is proven by execution rather than promised:** flipping the derived status through every member a non-empty adjudicator set can produce leaves `outcome`, `outcome_reason`, all seven condition verdicts, `precision_evaluable`, `meets_threshold` and the closure path BYTE-IDENTICAL, over both an otherwise-`CLEARED` generated population and a `BLOCKED` one (`-109`), with the fixtures asserted first to differ ONLY in the adjudicator field. **Why a disclosure and not a condition:** the honest answer today is *no*, and a condition would convert an honest *no* into a blocked gate that only a hiring decision could unblock — §2's external sign-off *"SHOULD be outside the implementing team"*, and **SHOULD is not MUST**. **The derived answer today is `NOT_INDEPENDENT`** — 31 of 31 live human judgements authored by `"XAgent007 (Engineering Lead)"`, who is also the tool's author — **and that is the correct output of the mechanism, not a failure of it.** **`argus/verdict/negative_assurance.py` is byte-unchanged and `InstrumentStatus` stays CLOSED at two:** that vocabulary bounds the INSTRUMENT per tool VERSION and is removed only by the gate clearing; this one bounds ONE adjudication run and changes whenever the record does, and merging them would repeat the run-grade/instrument-status confusion that enum's own docstring warns against. **The note attaches ONLY where the status is rendered from an adjudication record:** `argus/dogfood/proof_run.py::derive_gate_status` passes `precision=None` and reads no record, and the cartridge fold has golden keys rather than adjudicators — both are byte-unchanged and both render byte-identical output, asserted by comparing rendered strings. **No change-log version was taken a fourth time:** the amendment is a dated block under the existing V1.3, the 31 human judgements keep their provenance, `adjudication-record.json` is byte-unchanged, no role was filled, no disposition written, no member ratified and no detector run, and `DF-13-5-A` stays OPEN and UNSPENT.
...
**Stale-target-story enforcement** *(added 2026-08-26 by Story 17.5 / AC6–AC8)*. **The rule: no canonical `deferred-work.md` entry block's own `- target_story:` field may AFFIRMATIVELY name a story key `sprint-status.yaml` records as `done` — a `target_story` pointing at a story that is `done` is work with no container, recorded as work that is already scheduled.** It is enforced by **`tests/test_governance_record_integrity.py`** (`TC-ArgusAgent-DOCS-001-80`), over pure exported analyzers (`done_story_keys`, `ledger_target_pointers`, `is_affirmative_target`, `named_done_stories`, `stale_target_pointers`). **The population is narrowed by stated rule, never by convenience, and each narrowing carries a positive control over synthetic input:** the LANDMARK form is not a violation (a field reading `NONE — no story exists after 9.2` or `NONE — the next story that edits X` mentions a done story as a reference point, not as an owner — **18** of the 70 measured blocks, count asserted), and the DISPOSING-STORY form is not a violation (`DF-1-7-B` names Story 6.2 because Story 6.2 discharged it on 2026-06-29; excluded by name in `_DISPOSING_STORY_POINTERS`). **The historical population is registered in `_POINTS_AT_DONE_AT_LANDING` by name, dated 2026-08-26, owned by XAgent007 (Engineering Lead) (`AI-E9-8`), with a per-entry reason — and the registry can only SHRINK, and openness is never decided by `ledger_closed_ids`.** Both halves of that sentence are load-bearing. `-80` fails if a registered pair becomes clean, exactly as `TC-ArgusAgent-MAINT-001-04` treats a file no longer over the ceiling, so the registry cannot become a parking lot; and the obvious openness filter is refused because it is MEASURED defective for this question — at `b8eaeee` `ledger_closed_ids` reports 42 disposed ids of which `DF-13-5-A` (a future conditional, and a `Neither …` negation the lookbehind does not know) and `DF-12-3-A` (a half-disposition) are false positives, the third independent measurement of `DF-16-6-D`'s counted class. **A stale pointer on a disposed entry is still a stale pointer, so the rule needs no openness test to be true.** **Why this rule and not a reviewer instruction:** `DF-16-5-A`'s own body predicted one of these in terms — *"Pinning it to a story nobody has written yet is how `DF-14-3-H`'s `target_story: 13-5` went stale"* — and `DF-14-3-H` went stale anyway because nothing was watching; in the same epic, Story 17.3 CREATED a fresh instance of the twin defect in `argus/detectors/assertion_strength.py` one story before the story chartered to remove it. Knowing about a defect class does not prevent it (`DF-16-6-D`). Dispositions and the full measurement live in `deferred-work.md` under *"Story 17.5 dispositions — 2026-08-26"*. `TC-ArgusAgent-DOCS-001-77` asserts this registration is still present.
...
**Vacuity-corroboration enforcement** *(added 2026-08-17 by [sprint-change-proposal-2026-08-17.md](sprint-change-proposal-2026-08-17.md), implemented by Story 14.1)*. **The rule: a `vacuous_test_ast` finding is verdict-eligible ONLY on evidence that the asserted values do not derive from the SUT — NEVER on the mere presence of a mock.** Cross-cutting concern #6 has required *"`audited_deep` AST corroboration AND Prosecutor sign-off"* since this architecture was written; the Story 1.5 detector shipped **`AUDITED_SHALLOW` with no sign-off**, and its fact (b) reduced to `assertion_sites >= 1 and mock_sites >= 1` — which is *"the test constructs a mock"*, a fact the heuristic already had. **Measured on the day it was found, which is why this is a rule and not a preference:** over the ratified 5-repository corpus the rule class emitted **31** blocking findings and the named human adjudicated **0** of them true (26 FP / 5 BORDERLINE); across **1,836** heuristically-flagged tests in the two contributing members `ast_corroborated` was equivalent to `mock_sites >= 1` in **1,835** cases, so the corroboration step **added no evidence the heuristic did not already have** — it re-read one input and treated the agreement as confirmation. **The Epic-6 Prosecutor does not close this, and relying on it was the error:** `argus/verdict/prosecutor.py:56-57` leaves an ALREADY-verdict-eligible finding **UNCHANGED**, so sign-off gates only the *promotion* path (advisory → eligible), and the V1 call site at `pipeline.py:535` passes **no `sign_offs`** by design (the deterministic zero-token default). *"AST corroboration AND Prosecutor sign-off"* is therefore an `AND` only for the findings that do not need it, and an `OR` for the one class that does. **The moat is consequently enforced AT THE DETECTOR**, and the conservative default is restored: where the unresolved 1.4 edge set (DF-1-4-A) cannot establish fact (b), corroboration is **NOT granted** and the finding stays `vacuous_test_heuristic` / advisory. The V1 signal is **name-level and a proxy** — full dataflow-grounded assertion provenance is **UNSCHEDULED**, owner XAgent007 (Engineering Lead), its destination a scope change (`DF-14-1-A`) — and it is measured against three populations, never argued: the planted-vacuous cartridges (recall, incl. the `holdout_vacuous` anti-overfitting control), the 31 adjudicated locators (precision), and the whole flagged population (promotion rate). **AMENDED 2026-08-26 by Story 17.5 / `DN-17-5-12` — the pointer only, never the 2026-08-17 measurement.** The struck text stays legible per §3.4: ~~*"full dataflow-grounded assertion provenance remains **Story 6.2**'s scope"*~~. Story 6.2 has been `done` since 2026-06-29 and shipped a deterministic STRUCTURAL grounding fact instead, so that sentence was a live forward reference to a closed story — the exact class the **Stale-target-story enforcement** rule below exists to end, two blocks away, which is why it is corrected here rather than deferred. The corrected destination is recorded in `deferred-work.md` under *"Story 17.5 dispositions — 2026-08-26"*. ⛔ **This is a POINTER correction: no measurement, no rule text, no enforcing module and no test id in this block moves.**

**Anti-drift enforcement** *(added 2026-08-23, forward-only from Epic 17)*. **The rule: an epic's backlog is at most 20% process-derived — stories whose provenance in `epics.md` is only an `AI-E*` action item, an `SD-*` alert or a `DF-*` ledger entry — and every epic seed carries a `**Capability delivered:**` field naming what a user can do at the end that they could not at the start. `Nothing` is permissible only with a dated operator ruling cited on the line.** **Measured on the day it landed, which is why it is a rule and not a preference:** the process-derived share of an epic's stories ran **11% (Epic 12) -> 25% (13) -> 33% (14) -> 86% (Epic 16)**, and from Epic 13 onward **no epic seed's `**Covers:**` line names a single FR/NFR driver**. Over the 128 commits since `40cdb3c` (2026-08-16) `argus/` changed by **8,486 lines**, of which **6,483 (76%) are `argus/precision/`** — this project's own gate machinery, unreachable from `argus.cli` — while **`argus/cli.py` and `argus/commands/` changed by zero lines**. The last user-reachable feature shipped 2026-08-15 (Story 12.8). **This does NOT repeal the routing convention**: retrospective outputs are still filed as `AI-*` and deferred work as `DF-*` — that convention solved a real follow-through problem and is untouched. It budgets how much of a *delivery* epic they may consume; work over budget is scheduled as a named corrective epic, not forbidden. It is reported by **`scripts/check_meta_drift.py`** (§RD-1 / §CD-2), which classifies provenance from `epics.md` — the document where this project already records why a story exists — and never from a story file, whose §0 research legitimately cites every driver it re-measured. **The mechanism is ADVISORY and always exits 0 on a clean parse**: its predicates read prose and make a judgement, and a blocking prose-parser produces false failures that abort legitimate work — `TC-ArgusAgent-DOCS-001-78` went RED three times from prose in one day (`epic-14-retro-2026-08-18.md` §3.4). A non-zero exit means the script could not READ its inputs, never that a finding was made. The pre-rule state is frozen in **`meta-drift-baseline.md`**, which carries the warning that a row may never be added to silence a new finding. **Epics 1-16 are never classified** — their provenance is evidence, not debt (§3.4).

**Guard-fire ledger enforcement** *(added 2026-08-23; the RETIREMENT half is deliberately NOT yet in force)*. **The rule: every guard observed RED is recorded AUTOMATICALLY with its id and the sha it was observed at, and "caught a real defect" means one thing only — an observation that a human has since resolved against the sha of a fix which changes at least one non-test file.** A guard driven RED by its own author's executed mutation does **NOT** qualify: that is vacuity evidence, which this project already requires (`AI-E14-1`) and which is a different property. **Why the retirement half is withheld, measured:** the tree carries **1,251 distinct guard ids**, of which **967 (77%) are named nowhere outside their own test file** — no commit, no story, no retrospective — and **zero test files or scripts have ever been deleted** in 189 commits (`git log --diff-filter=D -- 'tests/*.py' 'scripts/*'` is empty). On that evidence *"has never caught anything"* is indistinguishable from *"nobody wrote it down"*, and a sunset rule would be a licence to delete inconvenient checks rather than a counterweight to unbounded promotion. **No guard may be retired under this rule until the ledger has run for two full epics**, and then only by a story that proposes it by name, with the operator ruling. Deletion is never automatic and never taken between stories. It is enforced by **`tests/conftest.py`** (`pytest_runtest_logreport`), which appends one JSONL observation per RED guard to `.argus/guard-fires.jsonl`. **The mechanism is AUTOMATIC by design and the reason is measured:** the manual alternative is the one `AI-E14-1`..`AI-E14-9` recorded as not-addressed across six consecutive retrospectives, so a rule depending on a human remembering to write a row is a rule that will not run. It writes to a **gitignored** path because a hook that dirtied the working tree would break `release_preflight`'s E1 refusal and `tests/test_dogfood_artifact_currency.py` — telemetry that fails the build it informs is worse than none. **It can never fail the suite:** every path is exception-wrapped and records nothing on error, because taking down 1,703 passing tests to capture a statistic is not a trade this project makes. Verified 2026-08-23 by executed probe: a synthetic `TC-`named failure was recorded with its id, nodeid and sha; an ordinary non-guard failure in the same run was correctly **not** recorded. Reported by `scripts/check_meta_drift.py` §GS-3, which proposes nothing for deletion.

**Ruling-index enforcement** *(added 2026-08-23; the reconciliation half is DECLINED, not deferred)*. **The rule: a new ruling is added to the ruling index as one line pointing at the story record that holds its reasoning, and its reasoning is not restated elsewhere.** **Measured:** `DN-3` is currently restated in **48** documents under the artifact folder, `AI-E1-1` in 33, `DF-13-5-A` in 27; and `§3.4 evidence immutability` — the rule cited **144 times** across this repository — has **no defining section in any indexed governance document**, only a subsection of a story record (`stories/10-1-release-status-must-cite-evidence.md` §C). Every copy must be kept consistent with every other, and that reconciliation is itself a work generator. **The existing restatements are NOT removed and this rule does not try**: §3.4 forbids erasing the originating text, so the index is additive and authoritative going forward only, which stops the count growing without destroying a byte. Reconciling the existing copies is **explicitly declined** rather than filed as future work, because filing it would create exactly the kind of unbounded process backlog this section exists to bound. Duplication is reported by `scripts/check_meta_drift.py` §RB-4.

**Outcome-scenario enforcement** *(added 2026-08-23)*. **The rule: a release decision requires a one-page scenario sheet — end-to-end runs of the SHIPPED CLI across the real user-facing configurations, each terminating in a state that matches what was actually produced.** **Measured, which is why this is a rule and not a preference:** the gates classified on 2026-08-23 run roughly **21 bookkeeping checks to 6 outcome checks**, and the committed `_bmad-output/audit-reports/final-verdict.md` reads `RELEASE_READY` with 0 blocking findings while the protocol §5 precision gate reads `BLOCKED` — both true, neither a release decision. It is enforced by **`scripts/acceptance_scenarios.py`**, which builds six hermetic fixture repositories from bytes in its own source (nothing fetched, no ratified corpus member read, no checkout depended on) and cross-checks **three independently produced surfaces** per scenario: the process exit code the OS saw, the stdout machine summary a scripted consumer parses, and the rendered `final-verdict.md` a human reads. **No expected verdict is hardcoded anywhere**, deliberately: a sheet of pinned verdicts degrades into a change detector that re-asserts whatever the tool currently does and must be edited every time a detector legitimately improves. Disagreement between the three surfaces is a defect in **any** direction — the class Epic 8 spent five stories removing from the verdict, checked at the outermost boundary where a user stands. **A surface that cannot be OBSERVED is `UNEVALUABLE`, never a silent pass** (the `release_preflight` three-outcome precedent). **Driven to both outcomes by executed mutation on the day it landed** (`AI-E14-1`): 6 of 6 scenarios agree at HEAD, and flipping the observed exit code away from the report's stated one turns them RED with the failing check named. **What it does NOT cover, stated rather than implied:** the published Action is exercised through its flag set, not a real Actions runner; and the deep pass only in its provider-unconfigured degradation, because release evidence must not depend on a paid third party. **The existing gates are DEMOTED, not deleted** — every one keeps the blocking power it has today; this sheet is the decision they inform.
